...
class cVideo
{
	public:
		...
		void SetDRMDelay(unsigned int delay);
		void SetVideoDelay(unsigned int delay);
		void HandleDRMMessage(int Event, void *pData);
		void HandleVideoMessage(void * hHandle, int Event, void *pData);
		VIDEO_DEFINITION   GetVideoDef(void);
		...
		bool ReceivedDRMDelay(void);
		bool ReceivedVideoDelay(void);
		void SetReceivedDRMDelay(bool Received);
		void SetReceivedVideoDelay(bool Received);
